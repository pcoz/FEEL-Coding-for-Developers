...
Machine learning models are great at spotting patterns — predicting churn, classifying images, scoring credit risk. But a raw prediction is never a business decision. Between a model's output (`churn_probability: 0.73`) and a business action ("offer a 20% retention discount") sits a gap filled with **business logic**: thresholds, overrides, compliance checks, and routing rules.

FEEL is built for exactly this gap. It takes ML model outputs, wraps them in business rules, and turns predictions into decisions — transparently, auditably, and without forcing a business analyst to crack open a Jupyter notebook.

This appendix walks through five integration patterns, each with a worked example you can adapt to your own projects.

---

## F.1 Pattern: Pre-Processing — Feature Engineering in FEEL

Before data reaches an ML model, it usually needs transformation. FEEL can compute derived features from raw business data -- and because the logic lives in a readable expression rather than a Python script, both the data scientist and the business analyst can see exactly what goes in.
...
Notice that every feature definition reads like plain language. If the business decides that "days since last order" should be capped at 365, or that "order frequency" should only count orders from the last 12 months, you change one FEEL expression. The model itself never needs retraining.

---

## F.2 Pattern: Post-Processing — Applying Business Rules to Model Outputs

This is the pattern you will reach for most often. The ML model produces a score or classification; FEEL expressions then layer business rules on top to decide what actually happens next.
...
For the input above (`churn_probability: 0.73`, `Tier: "Premium"`), Rule 4 matches, yielding `{action: "Loyalty reward", discount: 10}`.

This is the real payoff: the business analyst can adjust thresholds, add tiers, or swap out actions without touching the ML model. The data scientist can retrain the model without touching the business rules. The two sides evolve on their own schedules, by different teams, with zero coordination overhead.

---

## F.3 Pattern: PMML Integration

**PMML (Predictive Model Markup Language)** is an XML-based format for serialising ML models -- logistic regression, decision trees, neural networks, and more. DMN lets you invoke a PMML model directly from a boxed expression via an **external function**, so you can wire a trained model into your decision logic without writing glue code.

### How It Works

1. Train a model in Python (scikit-learn), R, or any ML framework you like.
2. Export it to PMML format (using libraries like `sklearn2pmml`, `r2pmml`, or `nyoka`).
3. Reference the PMML file from a DMN Business Knowledge Model (BKM).
4. Invoke the BKM from a FEEL expression -- the engine feeds your inputs to the PMML model and hands back the prediction.
...
The ML model provides the prediction; FEEL provides the business interpretation on top. Notice the override rule -- "existing customers get bumped up one band" -- sitting right there in the expression, visible to anyone reviewing the decision, testable in isolation, and changeable without retraining a single model.
...
**ONNX (Open Neural Network Exchange)** is a format for serialising neural networks and other ML models. If PMML is the workhorse for classical ML, ONNX is its counterpart for deep learning. DMN 1.5+ supports ONNX as an external function type, so the integration pattern looks almost identical.

### Worked Example: Image Classification Result to Business Decision

Imagine an ONNX model that classifies product images on a manufacturing line. The model outputs a label and a confidence score; FEEL decides what to do with the result:
...
The quality thresholds (`0.95` for auto-reject, `0.90` for auto-accept) are business parameters. When the quality team decides to tighten acceptance standards, they tweak a number in a FEEL expression -- no one needs to retrain the neural network or redeploy a container.

---

## F.5 Pattern: Hybrid Decision — Combining Rules, Tables, and ML

This is where everything comes together. A single decision model can combine FEEL expressions for computation, decision tables for classification, and ML models for prediction -- each doing what it does best.
...
Every component of the premium is traceable -- and that traceability is the whole point:
- The base rate came from the decision table (rule 2).
- The risk adjustment came from the ML model (37% risk score x 0.5 multiplier).
- The claims adjustment came from the FEEL expression (1 claim, no at-fault, so 1.15).
- The loyalty discount came from a simple FEEL conditional (4 years with us, so 5% off).

A regulator can audit each component independently. A data scientist can retrain the risk model without touching the base rates. A business analyst can adjust the loyalty discount tiers without going anywhere near the model. Everyone stays in their lane, and the decision stays explainable end to end.

---

## F.6 Taking the Noise Out of ML

One of the most underappreciated benefits of separating business rules from ML models is **noise reduction** -- in the data, in the model, and in the process. This section explains why that matters more than most teams realize.

### The Noise Problem

When business rules are baked into the training data or into the model itself, the model ends up learning things it should not need to learn.

**Consider a fraud detection model trained on raw transaction data.**

The training data includes transactions that were flagged as fraudulent *because of a business rule* -- say, "any transaction over $10,000 from a new account is automatically flagged." The model dutifully learns this threshold as a pattern. But that threshold is not a statistical signal; it is a policy decision someone made in a meeting. If the policy changes from $10,000 to $15,000, the model has to be retrained -- even though the underlying fraud patterns have not changed at all.
...
| Threshold change requires retraining | Threshold change requires editing a FEEL expression |
| Model accuracy includes "easy" rule-based cases, inflating metrics | Model accuracy reflects only genuinely ambiguous cases |
| "Why was this flagged?" yields an opaque model explanation | "Why?" yields "rule: amount > $10K from new account" or "model: 83% fraud probability" |
...
1. **Training data is cleaner.** Deterministic cases -- the ones decided by rules -- drop out of the training set. The model only sees cases where statistical learning actually adds value.

2. **The model is simpler.** It no longer wastes capacity learning obvious thresholds or policy-driven classifications. It focuses on the genuinely uncertain cases, which is where ML earns its keep.

3. **Predictions are more meaningful.** A fraud score of 0.8 now means "the model thinks this is likely fraud based on behavioural patterns," not "the transaction was over $10,000."

4. **Explanations are precise.** When a decision comes from a rule, the explanation is exact ("amount exceeded threshold"). When it comes from the model, the explanation is a model interpretation (SHAP values, feature importance). The two never get conflated.
...
An ML model is trained to predict loan approval. The training data includes historical decisions that were made partly by policy ("reject all applicants under 18") and partly by judgement. The model learns `age < 18 -> reject` as a strong feature -- even though that is not a statistical pattern. It is a legal requirement. The model has no business "learning" it; it should just be enforced.
...
The model never sees applicants under 18 or below the income floor -- those are rule-based rejections, handled before the model is even called. The model trains only on applicants who passed the eligibility gate, so it learns the patterns that actually matter: income stability, debt management, payment behaviour. You end up with a cleaner model, a faster training cycle, and a decision trail that any auditor can follow from start to finish.
...
FEEL's sweet spot in the ML pipeline is the **last mile**: taking a model's output and wrapping it in the business rules that turn a prediction into a decision. That last mile is where transparency, auditability, and business ownership matter most -- and it is exactly what FEEL was designed for.
